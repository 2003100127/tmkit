...
check-codestyle:
	poetry run isort --diff --check-only --settings-path pyproject.toml tmkit
	poetry run black --diff --check --config pyproject.toml tmkit
	poetry run pydoclint tmkit
...
check-safety:
	...
	poetry run bandit -ll --recursive tmkit tests
