...
my $sv_path = $ARGV[1];
if(-d $sv_path){
	print "The folder for saving results is confirmed. \n";
}else{
	mkdir($sv_path);
}
# print "save path is: $sv_path\n";

open SEQ, "$ARGV[0]" or die $!;
$sequence="";
while (<SEQ>) {
	chomp;
	$sequence=join(" ",$sequence,$_);
...
for($i=0; $i<4; $i++){
   print "     SURFACE $i:\n";
   my $custom_filename  = LIPS;
   my $path = $sv_path."$i.txt";
...
my $custom_filename  = LIPS;
my $path = $sv_path."$custom_filename.txt";
open(my $handle, ">", $path)    || die "can't open $path: $!";
binmode($handle);               # for raw; else set the encoding
foreach $i (keys(%sumim)){
        ...
        printf $handle "%4s %12.3f %10.3f %8.3f\n", $i, $avpim, $ave, $peim;
